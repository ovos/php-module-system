...
copyright in it for any **permitted purpose**. However, you may only
distribute the software according to [Distribution License](#distribution-license)
...
## A plain-language note (not part of the license)

Read it, run it, learn from it, write about it, build on it for a hobby
project, a paper or a classroom. Use it to make money and you need a separate
agreement — write to [office@ovos.at](mailto:office@ovos.at) and ask.

This does not restrain anyone from implementing the same *ideas*: cache
stampede protection with a distributed lock and a Pub/Sub wake-up, session
documents with per-path reads and per-value locks, or anything else described
here. Read it, understand it, build your own.
